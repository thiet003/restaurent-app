...
const DishesList = () => {
  ...
  const fetchDishes = async (page = 1) => {
    try {
      ...
      const apiUrl = `${process.env.REACT_APP_URL_API_ADMIN}/dishes?keyword=${encodeURIComponent(keyword)}&category=${encodeURIComponent(category)}&page=${page}`;
    ...
    <div className="dishes-list-container">
      ...
      <div class="search-container">
        <div class="search-box">
          ...
          <button className="search-button" onClick={searchDishes}>Tìm kiếm</button>
        </div>
        <div class="filter-container">
          <select class="filter-select" name="category" id="category-select" onChange={handleCategoryChange}>
      ...
      <table className="dishes-table">
        ...
        <tbody>
          {dishes.map((dish) => (
            <tr key={dish.dish_id}>
              <td><Link to={`/admin/v1/detail-dish/${dish.dish_id}`}>{dish.name}</Link></td>
              <td>
                <img
                  src={dish.thumbnail}
                  alt={dish.name}
